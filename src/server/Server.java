...
public class Server implements Runnable {
	...
	public void run() {
		...
		while (running) {
			...
			if (text.equals("raw")) {
				 ...
				 * Kicking
				 */
			} else if (text.startsWith("kick")) { // /kick /1244
				// /kick guinness
				String name = text.split(" ")[1]; // /kick guinness
				int id = -1;
				boolean number = true;
				try {
				...
				if (number) {
					...
					if (exists)
						...
						System.out.println("Client " + id + " doesn't exists. Check ID number...");
				} else {
					for (int i = 0; i < clients.size(); i++) {
						ServerClient c = clients.get(i);
				...
				System.out.println("This is a unknown command.");
				printHelp();
			}
	...
	private void printHelp() {
		System.out.println("Here is the list of all available commands");
		System.out.println("==========================================");
	...
	private void sendToAll(String message) {
		...
		for (int i = 0; i < clients.size(); i++) {
			...
			ServerClient client = clients.get(i); // here we got the server
													// client now
	...
	private void process(DatagramPacket packet) {
		...
		if (string.startsWith("/c/")) { // that is connection packet
			...
			clientResponse.add(Integer.parseInt(string.split("/i/|/e/")[1])); // /i/712/e/
